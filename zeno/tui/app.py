...
class ZenoTUI(App):
    ...
    def on_mount(self) -> None:
        ...
        log.write("[bold][bold]")
        log.write("[cyan]  ┌─┐        ┌─┐[/cyan]")
        log.write("[cyan]  └─┤  Zeno  ├─┘[/cyan]")
        log.write("[cyan]     └─┘[/cyan]")
        log.write("")
        log.write("[dim]Your on-device voice assistant[/dim]")
    ...
    def _write_zeno(self, text: str) -> None:
        self.query_one("#chat-log", RichLog).write(
            f"[dim]{time.strftime('%H:%M')}[/dim] [zeno-msg]Zeno: {text}[/zeno-msg]"
        )
    ...
    def _handle_input(self, raw: str) -> None:
        ...
        log = self.query_one("#chat-log", RichLog)
        log.write(f"[dim]{time.strftime('%H:%M')}[/dim] [user-msg]You: {raw}[/user-msg]")
    ...
    async def _process_and_show(self, text: str) -> None:
        ...
        log.write(f"[dim]{time.strftime('%H:%M')}[/dim] [zeno-msg]Zeno: {response}[/zeno-msg]")
        self._processing = False
        self.query_one("#message-input", Input).focus()
