...
class NewsSkill(BaseSkill):
    ...
    def handle(self, intent: str, entities: Entities, context: Context) -> str:
        ...
        try:
            ...
            return self._ddg_fallback(entities) or \
                self.say("Couldn't parse the news feed. Might be using alien formats.")
        except urllib.error.URLError:
            return self._ddg_fallback(entities) or \
                self.say("Can't reach the news feed right now. Check your connection.")
        except Exception as e:
            return self._ddg_fallback(entities) or self.say(f"News check failed: {e}")

    def _ddg_fallback(self, entities: Entities) -> str | None:
        """If the configured RSS feed is down, try a DuckDuckGo instant
        answer instead of failing outright. Returns None (not a message)
        if that doesn't turn up anything either, so the caller can show
        its own error."""
        try:
            from zeno.skills.search import _ddg_query, _extract_ddg_answer
            topic = entities.raw.get("text", "").strip() or "top headlines"
            data = _ddg_query(topic)
            if data:
                answer = _extract_ddg_answer(data)
                if answer:
                    return self.say(answer[:300])
        except Exception:
            pass
        return None
