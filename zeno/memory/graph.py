...
class KnowledgeGraph:
    """
    Note on threading: this singleton is shared by the Personaliser and
    ProactiveEngine, which run on a background thread in daemon mode
    alongside the (possibly multi-threaded) web server. Open with
    ...
    def query_raw(self, sql: str, params: tuple = ()):
        ...
        Prefer query()/get_usage_stats() for normal use; this exists for
        callers (like the personalisation engine) that need custom
        aggregation SQL without reaching into `_conn` directly and
        skipping the lock.
        """
        with self._lock:
