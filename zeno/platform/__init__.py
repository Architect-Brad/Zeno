...
def battery_status() -> dict | None:
    return get_provider().battery_status()


def set_torch(on: bool) -> bool:
    return get_provider().set_torch(on)


def send_sms(number: str, message: str) -> bool:
    return get_provider().send_sms(number, message)


def make_call(number: str) -> bool:
    return get_provider().make_call(number)
